<template>
	...
	>
		<div
			:class="[
				'flex flex-col h-full transition-all duration-200 ease-linear',
				extended ? 'w-1/4' : 'w-full',
			]"
		>
			<img
				:src="imageUrl"
				@click="toggleExtended"
				class="h-1/2 aspect-r object-fit flex-1 xl:object-center"
			/>
			<div class="w-full rounded-t-md transition-all duration-200 ease-linear">
				...
				>
					...
					<span
						class="font-bold w-full text-lg text-center transition-all duration-200 ease-linear"
						>{{ produit.nom }}</span
					>
				</div>
			</div>
			<div class="flex flex-row w-full p-2 divide-x-2 h-1/4">
				...
				<div class="flex flex-col h-full w-1/3 justify-center items-center">
					<label class="text-xl font-bold text-black antialiased">{{
						produit.quantite - produit.vendu || "0"
					}}</label>
					<label class="text-sm font-light text-gray-500 antialiased"
						>Unités</label
					>
		...
		<div
			class="flex-1 w-3/4 transition-all duration-700 ease-linear"
			v-if="extended"
		>
			<div class="w-full flex-row flex justify-between p-2">
				...
				<button
					...
					@click="isEditingFormule = !isEditingFormule"
					v-if="
						!isEditingFormule &&
						currentUser.Profile.Autorisations.find(
							(el) => el.nom == 'update-formule'
						)
					"
				>
					Modifier
				...
				<div class="flex flex-row space-x-2" v-if="isEditingFormule">
					...
					<button class="btn btn-sm btn-primary" @click="saveFormule">
						Enregistrer
					</button>
				</div>
			</div>
				...
				<tbody class="overflow-y-auto">
					...
					<tr v-if="isEditingFormule">
						<td class="form-control">
							...
							>
								<option
									v-for="matiere in matieres.filter(
										(el) =>
											!produit.Matieres.find((p) => p.id == el.id) &&
											!newProduit.Matieres.find((p) => p.id == el.id)
									)"
									:key="matiere.id"
									:value="matiere.id"
								>
						...
						<td class="">
							<input
								...
								v-model="newFormule.quantite"
							/>
						</td>
						<td>
							<button class="btn btn-sm btn-primary" @click="addMatiere">
								Ajouter
							</button>
						</td>
					</tr>
		...
		<Modal ref="editmodal" primaryColor="haja" @submit="update">
			...
			<template v-slot:body>
				<div class="flex flex-col w-full pb-5 overflow-y-auto">
					<div class="relative mt-[29rem] self-center mx-4">
						<img
							ref="avatarDisplay"
							class="w-full object-fit max-w-md rounded-md overflow-hidden"
							:src="imageUrl"
						/>
						<div class="absolute w-8 bottom-0 right-1" @click="pencilClick">
							<PencilIcon class="w-full" />
						</div>
						<input
							type="file"
							style="display: none"
...
export default {
    ...
    async update() {
      console.log()
			this.$store.dispatch("updateProduit", 
        {
          ...this.newProduit,
	...
	computed: {
		imageUrl() {
      console.log(this.newProduit.image);
      if (this.newProduit.image.includes('http')) return this.newProduit.image;
			return (
