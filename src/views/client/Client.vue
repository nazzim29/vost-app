<template>
	...
	>
		...
		<div class="flex-1 flex flex-col overflow-hidden">
			...
			>
				<ul
					class="w-full h-full overflow-hidden grid gap-x-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-5 px-3 items-center justify-center"
				>
					<ClientRow
...
export default {
	...
	computed: {
		...
		nbperpage() {
			if (window.innerWidth >= 640 && window.innerWidth < 768) return 2 * 9;
			else if (window.innerWidth >= 768 && window.innerWidth < 1280) return 4*9  ;
			else if (window.innerWidth > 1280) return 5 * 9;
			else return 1 * 9;
		},
		filters() {
			return [
